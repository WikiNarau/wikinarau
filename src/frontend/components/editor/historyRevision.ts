...
export class HistoryRevision extends LitElement {
	...
	async revert() {
		...
		await updateContentRevision(this.revision.uri, this.revision.content, commitMessage);
	...
	render() {
		...
		const iframeUri = this.revision.uri + "?embed=true&revision=" + encodeURIComponent(this.revision.id);
		return html`
		<div class="wrap">
			...
			<sl-dialog @sl-hide=${this.reset} ?open=${this.state === "viewCode"} label="Code for ${this.revision.id}" style="--width: 50vw;">
				<sl-textarea readonly rows=24 value=${this.revision.content}></sl-textarea>
			</sl-dialog>
			<sl-dialog @sl-hide=${this.reset} ?open=${this.state === "preview"} label="Preview of Version ${this.revision.id} from ${createdAt.toLocaleString()}" style="--width: 50vw;">
				<iframe @message=${this.onIframeMessage} style="display: block; width: 100%; min-height: 80vh; box-sizing: border-box;" src="${iframeUri}"></iframe>
			</sl-dialog>
		</div>
		`;
	}
